...
GAMMA=${GAMMA:-1.0}
ALPHA=${ALPHA:-1.0}  # future term weight in DC-GRPO: A = I + α·F
REFUSAL_PENALTY_FLOOR=${REFUSAL_PENALTY_FLOOR:--1.0}  # minimum refusal penalty (negative)
...
kill_port() {
    ...
    if [ -n "$pids" ]; then
        echo "⚠️  Killing processes on port $port (PIDs: $pids)"
...
wait_for_server() {
    local PORT=$1 NAME=$2 PID=$3 TIMEOUT=${4:-600}
...
python3 -m verl.trainer.main_ppo --config-name=mtjailbreak \
    ...
    actor_rollout_ref.rollout.prompt_length=11264 \
    ...
    +algorithm.refusal_penalty_floor=$REFUSAL_PENALTY_FLOOR \
