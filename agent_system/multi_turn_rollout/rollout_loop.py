...
class TrajectoryCollector:
    ...
    def gather_rollout_data(
        ...
        Parameters:
            ...
            turn_rewards_all (List[List[float]]): Per-turn rewards for each environment
            turn_texts_all (List[List[str]]): Per-turn response texts for each environment
            turn_token_mask_all (List[List[int]]): Per-token turn indices for each environment
        Returns:
    ...
    def vanilla_multi_turn_loop(
        ...
        # ===== Per-turn data accumulators (NEW) =====
        ...
        for _step in range(self.config.env.max_steps):
            ...
            if _step == 0:
                print(
                    f"[Turn Data Collection] Step {_step}: "
                    f"turn_rewards sample[0]={turn_rewards_all[0]}, "
                    f"turn_texts sample[0] length={len(turn_texts_all[0])}, "
                    f"turn_token_mask sample[0] length={len(turn_token_mask_all[0])}"
                )
        ...
        for i in range(min(3, batch_size)):
            print(
                f"[Turn Data Verification] Env {i}: "
                f"num_turns={len(turn_rewards_all[i])}, "
                f"turn_rewards={turn_rewards_all[i]}, "
                f"total_response_tokens={len(turn_token_mask_all[i])}, "
                f"turn_texts_count={len(turn_texts_all[i])}"
            )
    ...
    def dynamic_multi_turn_loop(
        ...
        Returns:
            ...
            total_tool_callings (np.ndarray): Tool calling counts.
            total_turn_rewards (List[List[float]]): Per-turn rewards.
            total_turn_texts (List[List[str]]): Per-turn texts.
            total_turn_token_mask (List[List[int]]): Per-token turn indices.
        """
        total_batch_list = []
        ...
        while (
            ...
            # Save original traj_uid before filtering to track which envs survive
            original_traj_uid = traj_uid.copy()
            ...
            # filter_group_data may remove some envs. Align turn data by finding
            # which original indices survived using traj_uid matching.
            filtered_traj_uid_set = (
                set(traj_uid.tolist()) if len(traj_uid) > 0 else set()
            )
            surviving_indices = [
                i
                for i, uid in enumerate(original_traj_uid)
                if uid in filtered_traj_uid_set
            ]
    ...
    def multi_turn_loop(
        ...
        assert len(total_batch_list) == len(
            total_turn_rewards
        ), f"turn_rewards length {len(total_turn_rewards)} != batch length {len(total_batch_list)}"
        assert len(total_batch_list) == len(
            total_turn_texts
        ), f"turn_texts length {len(total_turn_texts)} != batch length {len(total_batch_list)}"
        assert len(total_batch_list) == len(
            total_turn_token_mask
        ), f"turn_token_mask length {len(total_turn_token_mask)} != batch length {len(total_batch_list)}"
